...
const Cart = () => {
    ...
    const {products, currency, cartItems, removeFromCart, getCartCount, updateCartItem, 
        navigate, getCartAmmount} = useAppContext();
        
    const [cartArray, setCartArray] = useState([])
    const [setAddresses, setAetAddresses] = useState(dummyAddress)
            ...
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <p className="text-center">{currency ? currency : "$"} {product.offerPrice * product.quantity}</p>
                ...
                <button onClick={()=> {navigate('/products');scrollTo(0,0)}}
                 className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    ...
                    className="group-hover:-translate-x-1 trasnition "/>
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                { setAddresses.map((address, index)=>(
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    <p className="flex justify-between">
                        <span>Price</span><span>${getCartAmmount()}</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Shipping Fee</span><span className="text-green-600">Free</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Tax (2%)</span><span>${getCartAmmount() * 2 / 100}</span>
                    </p>
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Total Amount:</span><span>${getCartAmmount() + getCartAmmount() * 2 / 100}</span>
                    </p>
                </div>

                <button onCanPlay={placeOrder}
